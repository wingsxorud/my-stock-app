...
# [수정] 종목 리스트 획득 (이미지의 1개만 나오던 현상 해결)
...
with l_col:
    ...
    if st.button("🔄 200대 종목 풀 스캔"):
        ...
        bar = st.progress(0)
        pool_df = get_combined_stock_list()
...
with r_col:
    ...
    if search_input:
        stocks_df = get_combined_stock_list()
        # [핵심 수리] '삼성'이 포함된 모든 종목이 셀렉트 박스에 뜹니다.
