...
# [핵심 수리] 검색이 안 되던 이유인 종목 리스트 획득 방식을 가장 안정적으로 변경
...
def get_large_pool():
    try:
        ...
        # 모든 종목을 합치고 중복 제거
        combined = pd.concat([df_k, df_q])[['Code', 'Name']].drop_duplicates(subset=['Code'])
        return combined
    except:
...
with r_col:
    ...
    search_input = st.text_input("분석할 종목명을 입력하세요", placeholder="예: 삼성")
    
    if search_input:
        stocks_df = get_large_pool()
        # [핵심 수리] 키워드 '삼성'이 포함된 모든 종목을 셀렉트 박스에 담음
        matched = stocks_df[stocks_df['Name'].str.contains(search_input, case=False) | stocks_df['Code'].str.contains(search_input)]
        
        if not matched.empty:
            # 셀렉트 박스 구성 복구
            sel_options = ["--- 선택 ---"] + [f"{row['Name']} ({row['Code']})" for _, row in matched.iterrows()]
            sel = st.selectbox("🎯 종목 선택", sel_options)
