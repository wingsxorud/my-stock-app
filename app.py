...
# 1. 페이지 설정
st.set_page_config(page_title="주식 분석기 v8.0.6", page_icon="🚀", layout="wide")
...
# --- 메인 화면 ---
st.title("🚀 주식 분석기 v8.0.6 (KOSPI 50 골든 스캔)")

l_col, r_col = st.columns([1, 2])

# [왼쪽: KOSPI 50 스캐너]
with l_col:
    st.markdown('<div class="section-header">📡 KOSPI 50 실시간 스캔</div>', unsafe_allow_html=True)
    if st.button("🔄 레이더 새로고침"):
        st.cache_data.clear()
        st.rerun()
    
    with st.spinner("최정예 50개 종목 동시 분석 중..."):
        pool = get_kospi50_pool()
        ...
        if recs:
            ...
            st.warning("💡 현재 조건에 부합하는 종목이 없습니다. 새로고침을 눌러주세요.")
...
with r_col:
    ...
    if search_input:
        ...
        if not matched.empty:
            if len(matched) > 1:
                sel = st.selectbox("🎯 분석 대상을 정확히 선택하세요", ["--- 선택 ---"] + [f"{row['Name']} ({row['Code']})" for _, row in matched.iterrows()])
            ...
            if target_code:
                with st.spinner(f'🚀 {target_name} 정밀 분석 중...'):
                    ...
                    fig.update_layout(template='plotly_dark', height=350, margin=dict(l=0,r=0,t=0,b=0)); st.plotly_chart(fig, use_container_width=True)
