...
def analyze_news_sentiment(stock_name):
    ...
    try:
        # 구글 뉴스 검색 (최신순 느낌을 내기 위해 시간 정보 포함 시도)
        ...
        for i, item in enumerate(items):
            ...
            # 시간 가중치 계산 (i=0이 가장 최신 뉴스)
            # 최신 뉴스(0번)는 1.2배, 뒤로 갈수록 가중치 감소 (1.2, 1.1, 1.0, 0.9 ...)
...
if search_input:
    ...
    if not matched.empty:
        ...
        if target_code:
            st.markdown("---")
            with st.spinner(f'🚀 {target_name} 최신 뉴스 가중 분석 중...'):
