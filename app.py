...
# 1. 페이지 설정
st.set_page_config(page_title="행님 전용 주식 분석기 7.6.7", page_icon="💎", layout="wide", initial_sidebar_state="auto")
...
hist_start = st.sidebar.date_input("기록 조회 시작일", datetime.now() - timedelta(days=30)) # 넉넉하게 30일
hist_end = st.sidebar.date_input("기록 조회 종료일", datetime.now())

# --- 메인 ---
st.title("🚀 행님 전용 스마트 분석기 7.6.7")
search_input = st.text_input("🔍 종목명 또는 티커(TSLA, 005930 등) 입력", "")
...
if search_input:
    ...
    elif search_input.replace('.', '').isalpha():
        target_code = search_input.upper(); target_name = search_input.upper()

    if target_code:
        st.markdown("---")
        with st.spinner(f'🚀 {target_name} 분석 리포트 생성 중...'):
            ...
            if not df.empty:
                ...
                c2.metric("AI 적정가", f"{forecast.iloc[-1]['yhat']:,.2f}")
                c3.metric("예상 등락", f"{((forecast.iloc[-1]['yhat']-last_val)/last_val)*100:+.2f}%")
