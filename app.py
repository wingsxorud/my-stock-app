...
# 1. 페이지 설정
st.set_page_config(page_title="주식 분석기 v8.1.8", page_icon="🚀", layout="wide")
...
st.markdown("""
    ...
    .news-box { background-color: #262730; padding: 12px; border-radius: 10px; border-left: 4px solid #ff4b4b; margin-bottom: 8px; border: 1px solid #3e3e3e; }
    </style>
    """, unsafe_allow_html=True)

# [함수] 뉴스 감성 분석 및 리포트 수집
def analyze_news_sentiment(stock_name):
    ...
    try:
        ...
        for i, item in enumerate(items):
            ...
            if i < 5:
                dt_str = item.pubDate.text if item.pubDate else ""
                news_data.append({"title": title, "link": item.link.text, "source": item.source.text, "dt": dt_str})
    except: pass
...
with l_col:
    ...
    if st.session_state.recs:
        for r in st.session_state.recs:
            st.markdown(f"""<div class="scan-card"><b>{r['name']}</b> <span style="color:#28a745;">+{r['upside']:.2f}%</span><br>현재: {r['curr']:,}원 | <b>목표: {r['target']:,}원</b></div>""", unsafe_allow_html=True)
...
with r_col:
    ...
    if not matched.empty:
        ...
        if target_code and st.button(f"🚀 {target_name} 정밀 분석 시작"):
            with st.spinner(f'🚀 {target_name} 데이터 분석 중...'):
                ...
                curr_p = int(df['Close'].iloc[-1])
                ai_daily = round_to_tick(int(forecast[forecast['ds'] <= datetime.now()].iloc[-1]['yhat']))
                ...
                st.session_state.analysis_result = {
                    ...
                    "market_eval": market_eval, "news": news_list, "weight": weight, "df": df, "forecast": forecast
                }
    ...
    if st.session_state.analysis_result:
        ...
        # [복구된 그래프]
        st.write(f"### 📈 {res['name']} 주가 예측 그래프")
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=res['df'].index, y=res['df']['Close'], name='실제 주가', line=dict(color='#00ff00', width=2)))
        fig.add_trace(go.Scatter(x=res['forecast']['ds'], y=res['forecast']['yhat']*(1+res['weight']), name='AI 예측(뉴스반영)', line=dict(color='#ff00ff', dash='dash')))
        ...
        if res['news']:
            for n in res['news']:
                st.markdown(f"""
                <div class="news-box">
                    <span style="color:#00ffff; font-size:0.75rem;">[{n['source']}]</span> | <span style="color:#888; font-size:0.75rem;">{n['dt']}</span><br>
