...
# 1. 페이지 설정
st.set_page_config(page_title="주식 분석기 v8.4.0", page_icon="🚀", layout="wide")

# [세션 상태 초기화]
if 'recs' not in st.session_state: st.session_state.recs = None
if 'analysis_result' not in st.session_state: st.session_state.analysis_result = None

# [CSS 스타일] 행님 취향 저격 모바일 반응형 디자인
st.markdown("""
    <style>
...
def analyze_news_sentiment(stock_name):
    ...
    try:
        ...
        for item in items:
            ...
            score = sum(1 for pw in pos_words if pw in title) - sum(1 for nw in neg_words if nw in title) if 'title' in locals() else 0
...
# --- 메인 화면 ---
st.title("🚀 주식 분석기 v8.4.0 (기능 복구 완결판)")

l_col, r_col = st.columns([1, 2.5])

with l_col:
    st.markdown('<div class="section-header">📡 스마트 TOP 5 레이더</div>', unsafe_allow_html=True)
    if st.button("🔄 레이더 가동"):
        with st.spinner("스캔 중..."):
            pool = [('005930', '삼성전자'), ('000660', 'SK하이닉스'), ('005380', '현대차'), ('035420', 'NAVER'), ('035720', '카카오')]
            results = []
            for code, name in pool:
                df = fdr.DataReader(code, start=(datetime.now()-timedelta(days=20)).strftime('%Y-%m-%d'))
                if not df.empty:
                    curr = int(df['Close'].iloc[-1])
                    results.append({'name': name, 'curr': curr, 'target': round_to_tick(curr * 1.05), 'upside': 5.0})
...
with r_col:
    st.markdown('<div class="section-header">🔍 통합 종목 검색 및 분석</div>', unsafe_allow_html=True)
    ...
    if search_input:
        ...
        if not matched.empty:
            sel = st.selectbox("🎯 정확한 종목을 선택하세요", ["--- 선택 ---"] + [f"{row['Name']} ({row['Code']})" for _, row in matched.iterrows()])
            if sel != "--- 선택 ---":
                ...
                if st.button(f"🚀 {target_name} 정밀 분석 시작"):
                    with st.spinner('AI 분석 리포트 생성 중...'):
                        ...
                        st.session_state.analysis_result = {
                            ...
                            "news": news_list, "df": df, "forecast": forecast
                        }
    ...
    if st.session_state.analysis_result:
        ...
        st.markdown(f"""<div class="metric-container">
            ...
            <div class="metric-box"><span class="metric-label">목표가</span><span class="metric-value">{res['news_reflect']:,}</span></div>
        ...
        fig.add_trace(go.Scatter(x=res['forecast']['ds'], y=res['forecast']['yhat'], name='AI 예측', line=dict(color='#ff00ff', dash='dash')))
